...
def format_deg(decimal_deg):
    d = int(decimal_deg % 30) # राशि के अंदर का अंश
    m = int((decimal_deg - int(decimal_deg)) * 60)
    s = int(((decimal_deg - int(decimal_deg)) * 60 - m) * 60)
    return f"{d}° {m}' {s}\""
...
# --- मुख्य गणना इंजन ---
try:
    # 1. समय सुधार (IST to UTC)
    utc_time = tob.hour + tob.minute/60.0 - 5.5
    jd = swe.julday(dob.year, dob.month, dob.day, utc_time)
    
    # 2. लाहिरी अयनांश मोड को अनिवार्य रूप से सेट करना
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    
    # 3. लग्न गणना (Sidereal Flag के साथ ताकि मीन लग्न आए)
    ...
    for p_name, p_id in planets_map.items():
        # गणना में FLG_SIDEREAL का होना बहुत ज़रूरी है
    ...
    # --- प्रदर्शन ---
    st.write(f"### जातक: {name} | जन्म विवरण: {dob}, {time_str} ({
